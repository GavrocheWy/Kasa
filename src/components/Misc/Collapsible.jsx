...
const Collapsible = ({ openByDefault, title, content }) => {
    ...
    return (
        <article className={`collabsible ${isOpen ? 'is-open' : 'is-closed'}`}>
            <h2 onClick={() => setIsOpen(!isOpen)} className="collabsible--title">{title}<button className='collabsible--toggler'><HerringboneShape /></button></h2>
